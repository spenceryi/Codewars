/*
Odd or Even? Determine that! by ngntrgduc
...
function oddOrEven(n) {
    ...
    const firstHalf = Math.floor(n/2); // count of odd or even numbers in the series of numbers.
    const secondHalf = n - firstHalf; // count of opposite.

    if (isEven(firstHalf) && isEven(secondHalf)) {
        // If both counts are even, the sum will be even.
        return EVEN;
    } else if (!isEven(firstHalf) && !isEven(secondHalf)) {
        // If both counts are odd, the sum will be odd.
        return ODD;
    } else {
        // If only one count is even, the sum could be either even or odd.
        return EITHER;
    }
}
